...
install-venv: install install-systemd
	...
	$(SHELL) -c '($(PREFIX)/lib/catpuppetbridge/venv/bin/pip install -r requirements/prod-requirements.txt)'
	sed "s_ExecStart=$(PREFIX)/bin_ExecStart=/usr/local/lib/catpuppetbridge/venv/bin/python3 $(PREFIX)/bin_" -i /etc/systemd/system/catpuppetbridge.service
	systemctl daemon-reload

install-systemd: install-user
	cp catpuppetbridge.service /etc/systemd/system/
	sed "s_PREFIX_$(PREFIX)_" -i /etc/systemd/system/catpuppetbridge.service
	systemctl daemon-reload

install-user:
	adduser --system --group --home $(PREFIX)/lib/catpuppetbridge --disabled-login --shell /usr/sbin/nologin catbridge

uninstall:
	rm -rf $(PREFIX)/lib/catpuppetbridge
	rm -f $(PREFIX)/bin/catpuppetbridge
	rm -f /etc/catbridge.ini
	systemctl stop catpuppetbridge || echo 'No service to stop'
	rm -f /etc/systemd/system/catpuppetbridge.service
	systemctl daemon-reload
	deluser catbridge || echo 'No user to remove'
